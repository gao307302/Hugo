...
public class DistrictUpdate {
    ...
    List<String> getSqlList(String fileName, HashMap<String, Pair<String, Integer>> excelSqlMap) {
        ...
        XSSFSheet sheet = xssfWorkbook.getSheetAt(filePosQuarterDistrict.sheetNumUpdate);
        ...
        // 第一行是表头
        for (int row = filePosQuarterDistrict.rowNumUpdate; row <= maxRow; row++) {
            ...
            sb.append(" where date_year = '").append(year).append("'")
                    ...
                    .append(" and district_name = '").append(sheet.getRow(row).getCell(4).getStringCellValue()).append("'")
    ...
    public static void main(String[] args) {
        DistrictUpdate districtUpdate = new DistrictUpdate();
        FilePosQuarterDistrict filePosQuarterDistrict = new FilePosQuarterDistrict();
        HashMap<String, Pair<String, Integer>> sqlColumnTypeMap = InitUtils.getSqlColumnType(filePosQuarterDistrict.sqlColumnName);
        List<String> list = districtUpdate.getSqlList(filePosQuarterDistrict.updatedFile, sqlColumnTypeMap);
        System.out.println(list.size());
        for (String s :
                list) {
            InitUtils.saveAsFileWriter(s, filePosQuarterDistrict.outputFileUpdate);
        }
        if(filePosQuarterDistrict.newQuarter) {
            String extra;
            StringBuilder sb = new StringBuilder();
            extra = districtUpdate.getString(sb);
            InitUtils.saveAsFileWriter(extra, filePosQuarterDistrict.outputFileUpdate);
        }
    }
